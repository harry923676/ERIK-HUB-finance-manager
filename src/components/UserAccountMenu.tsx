  ...
  if (!isAuthenticated) {
    return (
      <button
        ...
        className="flex items-center gap-2 px-3 sm:px-3.5 py-1.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white rounded-full text-xs font-semibold shadow-xs hover:shadow-md active:scale-95 transition-all cursor-pointer shrink-0"
      >
        {/* Google G mini icon */}
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 p-1 sm:px-2.5 sm:py-1 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/80 dark:hover:bg-slate-700/80 border border-slate-200 dark:border-slate-700 transition-all cursor-pointer select-none"
      >
        {/* Avatar or Initial */}
      ...
      {isOpen && (
        <div 
          ...
          className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl z-50 p-4 text-slate-900 dark:text-white animate-in fade-in slide-in-from-top-2 duration-150"
        >
